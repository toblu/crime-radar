...
export const useForm = (submitFn: SubmitFn, validate?: ValidateFn) => {
  ...
  const [errorFields, setErrorFields] = React.useState<{[key: string]: boolean}>({})
  ...
  useDebouncedEffect(() => {
    setErrorFields(Object.fromEntries(Object.keys(errors).filter(field => !!values[field]).map(field => [field, true])));
  }, [errors], 400)
